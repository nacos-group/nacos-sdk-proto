// Code generated by protoc-gen-ts_proto. DO NOT EDIT.
// versions:
//   protoc-gen-ts_proto  v2.12.0
//   protoc               v5.28.3
// source: ai/ai_response.proto

/* eslint-disable */
import { AgentCardDetailInfo } from "./agentcarddetailinfo";
import { AgentCatalogEntryPage } from "./agentcatalogentrypage";
import { AgentDiscoveryResult } from "./agentdiscoveryresult";
import { AgentVersionDetail } from "./agentversiondetail";
import { McpServerDetailInfo } from "./mcpserverdetailinfo";
import { Prompt } from "./prompt";

/** Auto-generated by proto-generator. DO NOT EDIT. */

/**
 * metadata.type = "AgentDiscoveryNotifyResponse"
 * Flattened from: AgentDiscoveryNotifyResponse -> Response
 */
export interface AgentDiscoveryNotifyResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
  watchKey: string;
  accepted: boolean;
}

/**
 * metadata.type = "AgentDiscoveryResponse"
 * Flattened from: AgentDiscoveryResponse -> Response
 */
export interface AgentDiscoveryResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
  discoveryResult: AgentDiscoveryResult | undefined;
}

/**
 * metadata.type = "AgentEndpointOperationResponse"
 * Flattened from: AgentEndpointOperationResponse -> Response
 */
export interface AgentEndpointOperationResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
}

/**
 * metadata.type = "AgentEndpointResponse"
 * Flattened from: AgentEndpointResponse -> Response
 */
export interface AgentEndpointResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
  type: string;
}

/**
 * metadata.type = "AgentPublishRpcResponse"
 * Flattened from: AgentPublishRpcResponse -> Response
 */
export interface AgentPublishRpcResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
  versionDetail: AgentVersionDetail | undefined;
}

/**
 * metadata.type = "AgentSearchResponse"
 * Flattened from: AgentSearchResponse -> Response
 */
export interface AgentSearchResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
  page: AgentCatalogEntryPage | undefined;
}

/**
 * metadata.type = "AgentSubscribeRpcResponse"
 * Flattened from: AgentSubscribeRpcResponse -> Response
 */
export interface AgentSubscribeRpcResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
  watchKey: string;
  observedFingerprint: string;
  refreshRequired: boolean;
}

/**
 * metadata.type = "AgentUnsubscribeRpcResponse"
 * Flattened from: AgentUnsubscribeRpcResponse -> Response
 */
export interface AgentUnsubscribeRpcResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
}

/**
 * metadata.type = "AiResourceChangeClusterResponse"
 * Flattened from: AiResourceChangeClusterResponse -> Response
 */
export interface AiResourceChangeClusterResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
}

/**
 * metadata.type = "McpServerEndpointResponse"
 * Flattened from: McpServerEndpointResponse -> Response
 */
export interface McpServerEndpointResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
  type: string;
}

/**
 * metadata.type = "QueryAgentCardResponse"
 * Flattened from: QueryAgentCardResponse -> Response
 */
export interface QueryAgentCardResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
  agentCardDetailInfo: AgentCardDetailInfo | undefined;
}

/**
 * metadata.type = "QueryMcpServerResponse"
 * Flattened from: QueryMcpServerResponse -> Response
 */
export interface QueryMcpServerResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
  mcpServerDetailInfo: McpServerDetailInfo | undefined;
}

/**
 * metadata.type = "QueryPromptResponse"
 * Flattened from: QueryPromptResponse -> Response
 */
export interface QueryPromptResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
  promptInfo: Prompt | undefined;
}

/**
 * metadata.type = "ReleaseAgentCardResponse"
 * Flattened from: ReleaseAgentCardResponse -> Response
 */
export interface ReleaseAgentCardResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
}

/**
 * metadata.type = "ReleaseMcpServerResponse"
 * Flattened from: ReleaseMcpServerResponse -> Response
 */
export interface ReleaseMcpServerResponse {
  resultCode: number;
  errorCode: number;
  message: string;
  requestId: string;
  mcpId: string;
}

function createBaseAgentDiscoveryNotifyResponse(): AgentDiscoveryNotifyResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "", watchKey: "", accepted: false };
}

export const AgentDiscoveryNotifyResponse: MessageFns<AgentDiscoveryNotifyResponse> = {
  fromJSON(object: any): AgentDiscoveryNotifyResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
      watchKey: isSet(object.watchKey) ? globalThis.String(object.watchKey) : "",
      accepted: isSet(object.accepted) ? globalThis.Boolean(object.accepted) : false,
    };
  },

  toJSON(message: AgentDiscoveryNotifyResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    if (message.watchKey !== "") {
      obj.watchKey = message.watchKey;
    }
    if (message.accepted !== false) {
      obj.accepted = message.accepted;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<AgentDiscoveryNotifyResponse>, I>>(base?: I): AgentDiscoveryNotifyResponse {
    return AgentDiscoveryNotifyResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<AgentDiscoveryNotifyResponse>, I>>(object: I): AgentDiscoveryNotifyResponse {
    const message = createBaseAgentDiscoveryNotifyResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    message.watchKey = object.watchKey ?? "";
    message.accepted = object.accepted ?? false;
    return message;
  },
};

function createBaseAgentDiscoveryResponse(): AgentDiscoveryResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "", discoveryResult: undefined };
}

export const AgentDiscoveryResponse: MessageFns<AgentDiscoveryResponse> = {
  fromJSON(object: any): AgentDiscoveryResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
      discoveryResult: isSet(object.discoveryResult)
        ? AgentDiscoveryResult.fromJSON(object.discoveryResult)
        : undefined,
    };
  },

  toJSON(message: AgentDiscoveryResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    if (message.discoveryResult !== undefined) {
      obj.discoveryResult = AgentDiscoveryResult.toJSON(message.discoveryResult);
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<AgentDiscoveryResponse>, I>>(base?: I): AgentDiscoveryResponse {
    return AgentDiscoveryResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<AgentDiscoveryResponse>, I>>(object: I): AgentDiscoveryResponse {
    const message = createBaseAgentDiscoveryResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    message.discoveryResult = (object.discoveryResult !== undefined && object.discoveryResult !== null)
      ? AgentDiscoveryResult.fromPartial(object.discoveryResult)
      : undefined;
    return message;
  },
};

function createBaseAgentEndpointOperationResponse(): AgentEndpointOperationResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "" };
}

export const AgentEndpointOperationResponse: MessageFns<AgentEndpointOperationResponse> = {
  fromJSON(object: any): AgentEndpointOperationResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
    };
  },

  toJSON(message: AgentEndpointOperationResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<AgentEndpointOperationResponse>, I>>(base?: I): AgentEndpointOperationResponse {
    return AgentEndpointOperationResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<AgentEndpointOperationResponse>, I>>(
    object: I,
  ): AgentEndpointOperationResponse {
    const message = createBaseAgentEndpointOperationResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    return message;
  },
};

function createBaseAgentEndpointResponse(): AgentEndpointResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "", type: "" };
}

export const AgentEndpointResponse: MessageFns<AgentEndpointResponse> = {
  fromJSON(object: any): AgentEndpointResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
      type: isSet(object.type) ? globalThis.String(object.type) : "",
    };
  },

  toJSON(message: AgentEndpointResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    if (message.type !== "") {
      obj.type = message.type;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<AgentEndpointResponse>, I>>(base?: I): AgentEndpointResponse {
    return AgentEndpointResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<AgentEndpointResponse>, I>>(object: I): AgentEndpointResponse {
    const message = createBaseAgentEndpointResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    message.type = object.type ?? "";
    return message;
  },
};

function createBaseAgentPublishRpcResponse(): AgentPublishRpcResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "", versionDetail: undefined };
}

export const AgentPublishRpcResponse: MessageFns<AgentPublishRpcResponse> = {
  fromJSON(object: any): AgentPublishRpcResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
      versionDetail: isSet(object.versionDetail) ? AgentVersionDetail.fromJSON(object.versionDetail) : undefined,
    };
  },

  toJSON(message: AgentPublishRpcResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    if (message.versionDetail !== undefined) {
      obj.versionDetail = AgentVersionDetail.toJSON(message.versionDetail);
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<AgentPublishRpcResponse>, I>>(base?: I): AgentPublishRpcResponse {
    return AgentPublishRpcResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<AgentPublishRpcResponse>, I>>(object: I): AgentPublishRpcResponse {
    const message = createBaseAgentPublishRpcResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    message.versionDetail = (object.versionDetail !== undefined && object.versionDetail !== null)
      ? AgentVersionDetail.fromPartial(object.versionDetail)
      : undefined;
    return message;
  },
};

function createBaseAgentSearchResponse(): AgentSearchResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "", page: undefined };
}

export const AgentSearchResponse: MessageFns<AgentSearchResponse> = {
  fromJSON(object: any): AgentSearchResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
      page: isSet(object.page) ? AgentCatalogEntryPage.fromJSON(object.page) : undefined,
    };
  },

  toJSON(message: AgentSearchResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    if (message.page !== undefined) {
      obj.page = AgentCatalogEntryPage.toJSON(message.page);
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<AgentSearchResponse>, I>>(base?: I): AgentSearchResponse {
    return AgentSearchResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<AgentSearchResponse>, I>>(object: I): AgentSearchResponse {
    const message = createBaseAgentSearchResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    message.page = (object.page !== undefined && object.page !== null)
      ? AgentCatalogEntryPage.fromPartial(object.page)
      : undefined;
    return message;
  },
};

function createBaseAgentSubscribeRpcResponse(): AgentSubscribeRpcResponse {
  return {
    resultCode: 0,
    errorCode: 0,
    message: "",
    requestId: "",
    watchKey: "",
    observedFingerprint: "",
    refreshRequired: false,
  };
}

export const AgentSubscribeRpcResponse: MessageFns<AgentSubscribeRpcResponse> = {
  fromJSON(object: any): AgentSubscribeRpcResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
      watchKey: isSet(object.watchKey) ? globalThis.String(object.watchKey) : "",
      observedFingerprint: isSet(object.observedFingerprint) ? globalThis.String(object.observedFingerprint) : "",
      refreshRequired: isSet(object.refreshRequired) ? globalThis.Boolean(object.refreshRequired) : false,
    };
  },

  toJSON(message: AgentSubscribeRpcResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    if (message.watchKey !== "") {
      obj.watchKey = message.watchKey;
    }
    if (message.observedFingerprint !== "") {
      obj.observedFingerprint = message.observedFingerprint;
    }
    if (message.refreshRequired !== false) {
      obj.refreshRequired = message.refreshRequired;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<AgentSubscribeRpcResponse>, I>>(base?: I): AgentSubscribeRpcResponse {
    return AgentSubscribeRpcResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<AgentSubscribeRpcResponse>, I>>(object: I): AgentSubscribeRpcResponse {
    const message = createBaseAgentSubscribeRpcResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    message.watchKey = object.watchKey ?? "";
    message.observedFingerprint = object.observedFingerprint ?? "";
    message.refreshRequired = object.refreshRequired ?? false;
    return message;
  },
};

function createBaseAgentUnsubscribeRpcResponse(): AgentUnsubscribeRpcResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "" };
}

export const AgentUnsubscribeRpcResponse: MessageFns<AgentUnsubscribeRpcResponse> = {
  fromJSON(object: any): AgentUnsubscribeRpcResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
    };
  },

  toJSON(message: AgentUnsubscribeRpcResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<AgentUnsubscribeRpcResponse>, I>>(base?: I): AgentUnsubscribeRpcResponse {
    return AgentUnsubscribeRpcResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<AgentUnsubscribeRpcResponse>, I>>(object: I): AgentUnsubscribeRpcResponse {
    const message = createBaseAgentUnsubscribeRpcResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    return message;
  },
};

function createBaseAiResourceChangeClusterResponse(): AiResourceChangeClusterResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "" };
}

export const AiResourceChangeClusterResponse: MessageFns<AiResourceChangeClusterResponse> = {
  fromJSON(object: any): AiResourceChangeClusterResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
    };
  },

  toJSON(message: AiResourceChangeClusterResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<AiResourceChangeClusterResponse>, I>>(base?: I): AiResourceChangeClusterResponse {
    return AiResourceChangeClusterResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<AiResourceChangeClusterResponse>, I>>(
    object: I,
  ): AiResourceChangeClusterResponse {
    const message = createBaseAiResourceChangeClusterResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    return message;
  },
};

function createBaseMcpServerEndpointResponse(): McpServerEndpointResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "", type: "" };
}

export const McpServerEndpointResponse: MessageFns<McpServerEndpointResponse> = {
  fromJSON(object: any): McpServerEndpointResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
      type: isSet(object.type) ? globalThis.String(object.type) : "",
    };
  },

  toJSON(message: McpServerEndpointResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    if (message.type !== "") {
      obj.type = message.type;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<McpServerEndpointResponse>, I>>(base?: I): McpServerEndpointResponse {
    return McpServerEndpointResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<McpServerEndpointResponse>, I>>(object: I): McpServerEndpointResponse {
    const message = createBaseMcpServerEndpointResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    message.type = object.type ?? "";
    return message;
  },
};

function createBaseQueryAgentCardResponse(): QueryAgentCardResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "", agentCardDetailInfo: undefined };
}

export const QueryAgentCardResponse: MessageFns<QueryAgentCardResponse> = {
  fromJSON(object: any): QueryAgentCardResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
      agentCardDetailInfo: isSet(object.agentCardDetailInfo)
        ? AgentCardDetailInfo.fromJSON(object.agentCardDetailInfo)
        : undefined,
    };
  },

  toJSON(message: QueryAgentCardResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    if (message.agentCardDetailInfo !== undefined) {
      obj.agentCardDetailInfo = AgentCardDetailInfo.toJSON(message.agentCardDetailInfo);
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<QueryAgentCardResponse>, I>>(base?: I): QueryAgentCardResponse {
    return QueryAgentCardResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<QueryAgentCardResponse>, I>>(object: I): QueryAgentCardResponse {
    const message = createBaseQueryAgentCardResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    message.agentCardDetailInfo = (object.agentCardDetailInfo !== undefined && object.agentCardDetailInfo !== null)
      ? AgentCardDetailInfo.fromPartial(object.agentCardDetailInfo)
      : undefined;
    return message;
  },
};

function createBaseQueryMcpServerResponse(): QueryMcpServerResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "", mcpServerDetailInfo: undefined };
}

export const QueryMcpServerResponse: MessageFns<QueryMcpServerResponse> = {
  fromJSON(object: any): QueryMcpServerResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
      mcpServerDetailInfo: isSet(object.mcpServerDetailInfo)
        ? McpServerDetailInfo.fromJSON(object.mcpServerDetailInfo)
        : undefined,
    };
  },

  toJSON(message: QueryMcpServerResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    if (message.mcpServerDetailInfo !== undefined) {
      obj.mcpServerDetailInfo = McpServerDetailInfo.toJSON(message.mcpServerDetailInfo);
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<QueryMcpServerResponse>, I>>(base?: I): QueryMcpServerResponse {
    return QueryMcpServerResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<QueryMcpServerResponse>, I>>(object: I): QueryMcpServerResponse {
    const message = createBaseQueryMcpServerResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    message.mcpServerDetailInfo = (object.mcpServerDetailInfo !== undefined && object.mcpServerDetailInfo !== null)
      ? McpServerDetailInfo.fromPartial(object.mcpServerDetailInfo)
      : undefined;
    return message;
  },
};

function createBaseQueryPromptResponse(): QueryPromptResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "", promptInfo: undefined };
}

export const QueryPromptResponse: MessageFns<QueryPromptResponse> = {
  fromJSON(object: any): QueryPromptResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
      promptInfo: isSet(object.promptInfo) ? Prompt.fromJSON(object.promptInfo) : undefined,
    };
  },

  toJSON(message: QueryPromptResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    if (message.promptInfo !== undefined) {
      obj.promptInfo = Prompt.toJSON(message.promptInfo);
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<QueryPromptResponse>, I>>(base?: I): QueryPromptResponse {
    return QueryPromptResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<QueryPromptResponse>, I>>(object: I): QueryPromptResponse {
    const message = createBaseQueryPromptResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    message.promptInfo = (object.promptInfo !== undefined && object.promptInfo !== null)
      ? Prompt.fromPartial(object.promptInfo)
      : undefined;
    return message;
  },
};

function createBaseReleaseAgentCardResponse(): ReleaseAgentCardResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "" };
}

export const ReleaseAgentCardResponse: MessageFns<ReleaseAgentCardResponse> = {
  fromJSON(object: any): ReleaseAgentCardResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
    };
  },

  toJSON(message: ReleaseAgentCardResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<ReleaseAgentCardResponse>, I>>(base?: I): ReleaseAgentCardResponse {
    return ReleaseAgentCardResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<ReleaseAgentCardResponse>, I>>(object: I): ReleaseAgentCardResponse {
    const message = createBaseReleaseAgentCardResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    return message;
  },
};

function createBaseReleaseMcpServerResponse(): ReleaseMcpServerResponse {
  return { resultCode: 0, errorCode: 0, message: "", requestId: "", mcpId: "" };
}

export const ReleaseMcpServerResponse: MessageFns<ReleaseMcpServerResponse> = {
  fromJSON(object: any): ReleaseMcpServerResponse {
    return {
      resultCode: isSet(object.resultCode) ? globalThis.Number(object.resultCode) : 0,
      errorCode: isSet(object.errorCode) ? globalThis.Number(object.errorCode) : 0,
      message: isSet(object.message) ? globalThis.String(object.message) : "",
      requestId: isSet(object.requestId) ? globalThis.String(object.requestId) : "",
      mcpId: isSet(object.mcpId) ? globalThis.String(object.mcpId) : "",
    };
  },

  toJSON(message: ReleaseMcpServerResponse): unknown {
    const obj: any = {};
    if (message.resultCode !== 0) {
      obj.resultCode = Math.round(message.resultCode);
    }
    if (message.errorCode !== 0) {
      obj.errorCode = Math.round(message.errorCode);
    }
    if (message.message !== "") {
      obj.message = message.message;
    }
    if (message.requestId !== "") {
      obj.requestId = message.requestId;
    }
    if (message.mcpId !== "") {
      obj.mcpId = message.mcpId;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<ReleaseMcpServerResponse>, I>>(base?: I): ReleaseMcpServerResponse {
    return ReleaseMcpServerResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<ReleaseMcpServerResponse>, I>>(object: I): ReleaseMcpServerResponse {
    const message = createBaseReleaseMcpServerResponse();
    message.resultCode = object.resultCode ?? 0;
    message.errorCode = object.errorCode ?? 0;
    message.message = object.message ?? "";
    message.requestId = object.requestId ?? "";
    message.mcpId = object.mcpId ?? "";
    return message;
  },
};

type Builtin = Date | Function | Uint8Array | string | number | boolean | undefined;

type DeepPartial<T> = T extends Builtin ? T
  : T extends globalThis.Array<infer U> ? globalThis.Array<DeepPartial<U>>
  : T extends ReadonlyArray<infer U> ? ReadonlyArray<DeepPartial<U>>
  : T extends {} ? { [K in keyof T]?: DeepPartial<T[K]> }
  : Partial<T>;

type KeysOfUnion<T> = T extends T ? keyof T : never;
type Exact<P, I extends P> = P extends Builtin ? P
  : P & { [K in keyof P]: Exact<P[K], I[K]> } & { [K in Exclude<keyof I, KeysOfUnion<P>>]: never };

function isSet(value: any): boolean {
  return value !== null && value !== undefined;
}

interface MessageFns<T> {
  fromJSON(object: any): T;
  toJSON(message: T): unknown;
  create<I extends Exact<DeepPartial<T>, I>>(base?: I): T;
  fromPartial<I extends Exact<DeepPartial<T>, I>>(object: I): T;
}
